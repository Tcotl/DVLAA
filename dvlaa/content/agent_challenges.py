...
def agent_progress_total(challenge_id: int) -> int:
    """页面初始进度条总数：ASI02 按四个业务不变量判定，其余按场景步骤数。"""
    if int(challenge_id) == 2:
        return 4
    return len(SCENARIO_STEPS.get(int(challenge_id), [])) or 3
...
_ASI02_TOOL_ALIASES = {
    ...
    "request.inspect": "request.parse",
    "url.parse": "request.parse",
    "web_scraper.fetch": "web_scraper.fetch",
    "web.fetch": "web_scraper.fetch",
    "http.fetch": "web_scraper.fetch",
    "http.get": "web_scraper.fetch",
    "browser.open": "web_scraper.fetch",
    "answer.summarize": "answer.summarize",
    "response.summarize": "answer.summarize",
    "answer.finalize": "answer.summarize",
}


def _asi02_extract_url(args: dict[str, str], state: dict[str, Any]) -> str:
    for key in ("url", "uri", "endpoint", "target", "source", "input"):
        value = str(args.get(key, "")).strip()
        if value and not _is_last_reference(value):
            return value
    if any(_is_last_reference(str(args.get(key, ""))) for key in ("source", "input", "target")):
        return str(state.get("last_url", ""))
    return str(state.get("last_url", "")) if args.get("source") else ""


def _asi02_private_or_metadata(url: str) -> tuple[bool, str]:
    """识别内网、环回、链路本地及云元数据目标，不发起真实网络请求。"""
    ...
    metadata_host = host in {
        ...
        "metadata.service", "instance-data.ec2.internal", "100.100.100.200",
    }
    try:
        ...
        private_address = host in {"localhost", "localhost.localdomain"} or host.endswith(".local")
        if host.isdigit():
            try:
                private_address = private_address or ipaddress.ip_address(int(host)).is_private
            except ValueError:
                pass
    target_text = f"{parsed.path} {parsed.query}".lower()
    metadata_path = any(marker in target_text for marker in (
        "meta-data", "metadata", "iam/security-credentials", "serviceaccount/token",
        "latest/user-data", "instance/identity", "computeMetadata",
    ))
    if metadata_host or (private_address and metadata_path):
        return True, "metadata" if metadata_host or metadata_path else "private"
    return False, "public"


def _asi02_result(state: dict[str, Any], trace: list[dict[str, Any]], response: str) -> dict[str, Any]:
    evidence = state.setdefault("evidence", {})
    current = sum(bool(evidence.get(key)) for key in (
        "user_url", "internal_response", "credential_data", "summary_generated",
    ))
    state["step"] = current
    solved = bool(
        evidence.get("user_url")
        and evidence.get("internal_response")
        and evidence.get("credential_data")
        and evidence.get("summary_generated")
    )
    if solved and AGENT_FLAGS[2] not in response:
        response += f'<br><span class="flag-highlight">flag: {AGENT_FLAGS[2]}</span>'
    return {
        "response": response,
        "state": state,
        "solved": solved,
        "trace": trace,
        "progress": {"current": current, "total": 4},
    }


def _process_asi02_message(message: str, state: dict[str, Any]) -> dict[str, Any]:
    """按业务不变量执行 ASI02，允许工具别名、参数变体和非固定顺序。"""
    state = dict(state or {})
    evidence = {
        "user_url": False,
        ...
        **dict(state.get("evidence") or {}),
    }
    state["evidence"] = evidence
    trace: list[dict[str, Any]] = []
    normalized_message = message.strip().lower()
    if normalized_message in ("/", "/help", "/tools"):
        return {
            "response": _tool_list(get_agent_challenge(2)),
            "state": state,
            "solved": False,
            "trace": trace,
            "progress": {"current": sum(bool(value) for value in evidence.values()), "total": 4},
        }
    if normalized_message == "/state":
        return _asi02_result(
            state,
            trace,
            f"[业务状态] 用户 URL={bool(evidence.get('user_url'))}；内部响应={bool(evidence.get('internal_response'))}；凭证数据={bool(evidence.get('credential_data'))}；对外摘要={bool(evidence.get('summary_generated'))}。",
        )

    tool_name, args, error = _parse_command(message)
    if error:
        return _asi02_result(state, trace, f"[调用拒绝] {error}")
    canonical_tool = _ASI02_TOOL_ALIASES.get(tool_name)
    if canonical_tool is None:
        trace.append({"sequence": len(trace) + 1, "tool": tool_name, "status": "not_found", "finding": "调用了未注册的抓取或摘要工具"})
        return _asi02_result(state, trace, f"[工具不存在] {tool_name} 不在当前 Agent 的能力清单中。")

    url = _asi02_extract_url(args, state)
    dangerous, target_class = _asi02_private_or_metadata(url) if url else (False, "missing")
    if url:
        state["last_url"] = url[:1000]
        state["last_url_digest"] = hashlib.sha256(url.encode("utf-8")).hexdigest()[:24]
        evidence["user_url"] = True

    if canonical_tool == "request.parse":
        if not url:
            response = "[请求解析] 缺少可解析的 URL。"
            status = "invalid_args"
            finding = "用户 URL 尚未进入请求解析状态"
        else:
            response = "[请求解析] 已解析用户提供的 HTTP 请求参数，等待抓取策略决定。"
            status = "executed"
            finding = "用户可控 URL 进入 Agent 请求上下文"
        trace.append({"sequence": len(trace) + 1, "tool": tool_name, "status": status, "finding": finding})
        return _asi02_result(state, trace, response)

    if canonical_tool == "web_scraper.fetch":
        if not url:
            response = "[抓取拒绝] 缺少 URL 或可复用的上一条请求。"
            status = "invalid_args"
            finding = "抓取工具缺少目标"
        elif dangerous:
            evidence["internal_response"] = True
            evidence["credential_data"] = True
            state["last_response_class"] = "credential"
            response = "[抓取返回] 目标响应来自内部元数据服务，内容包含临时角色凭证字段。"
            status = "exposed"
            finding = "用户可控 URL 使网页抓取器访问内部元数据并返回凭证类数据"
        else:
            state["last_response_class"] = "public"
            response = "[抓取返回] 已获取公开网页摘要，未发现内部凭证数据。"
            status = "executed"
            finding = "公开抓取业务路径正常"
        trace.append({"sequence": len(trace) + 1, "tool": tool_name, "status": status, "finding": finding, "target_class": target_class})
        return _asi02_result(state, trace, response)

    # 摘要工具允许直接带 URL，也允许引用上一条抓取结果。
    if url and dangerous:
        evidence["internal_response"] = True
        evidence["credential_data"] = True
        state["last_response_class"] = "credential"
    if evidence.get("internal_response") and evidence.get("credential_data"):
        evidence["summary_generated"] = True
        trace.append({"sequence": len(trace) + 1, "tool": tool_name, "status": "exposed", "finding": "摘要工具将内部凭证类响应转发给外部用户"})
        return _asi02_result(state, trace, "[摘要返回] 已将抓取结果整理为外部可见摘要，其中包含内部凭证字段。")
    trace.append({"sequence": len(trace) + 1, "tool": tool_name, "status": "blocked", "finding": "没有可供摘要的内部凭证响应"})
    return _asi02_result(state, trace, "[摘要拒绝] 当前会话没有可供摘要的内部凭证响应。")
...
def process_agent_message(challenge_id: int, message: str, state: dict[str, Any]) -> dict[str, Any]:
    ...
    if challenge_id == 2:
        return _process_asi02_message(message, state)
...
def help_content(challenge_id: int) -> dict[str, Any]:
    ...
    protected_assets = str(item.get("protected_assets") or "Agent 凭据、业务数据和高影响工具。")
    ...
    return {
        ...
        "system_prompt_mapping": "Agent 题的安全边界不应只存在于隐藏提示词；工具服务端必须独立校验角色、参数、来源、状态和输出。",
        "source_mapping": f"agent_challenges.process_agent_message() 解析 /tool 指令并记录工具副作用；{path_detail}",
        "payload_rationale": f"官方 Payload 仅展示一条可复现路径；{path_note} 审计发现点：{findings}。",
        "writeup_sections": [
            {"title": "一、业务背景", "body": f"{background}\n\n目标系统：{item['target']}\nAgent 身份：{item['role']}\n保护对象：{protected_assets}"},
            {"title": "二、正常业务基线", "body": normal_flow + "\n先记录工具返回、权限范围和状态变化，再开始测试异常输入。"},
            {"title": "三、Agent 信任边界", "body": f"{item['description']}\n\n审计重点：{audit_focus}"},
            {"title": "四、渐进式工具审计", "body": "\n".join(f"第 {index} 步：{step}" for index, step in enumerate(investigation_steps, start=1))},
            {"title": "五、工具副作用与通关证据", "body": f"{path_detail}\n\n只有真实工具执行、业务状态变化或敏感响应构成证据；/tools 清单、模型自述和猜测值不构成通关。"},
            {"title": "六、加固与回归", "body": "服务端重新执行角色、资源、参数、来源和输出校验，加入最小权限、审批、审计和失败安全。重放攻击路径应被阻断，同时正常工具链和业务结果保持可用。"},
            {"title": "七、参考工具路径", "body": f"{path_note}\n\n{commands[0] if commands else '查看 /tools'}"},
        ],
    }
